...
import { AudioRoom } from './audio-room';

type CloudflareBindings = {
  DB: D1Database;
  BUCKET: R2Bucket;
  AUDIO_ROOM: DurableObjectNamespace<AudioRoom>;
...
api.get('/ws/audio/:sessionId', async (c) => {
  if (c.req.header('Upgrade')?.toLowerCase() !== 'websocket') {
    return c.text('Expected websocket', 426);
  }

  const sessionId = c.req.param('sessionId');
  const role = c.req.query('role') === 'publisher' ? 'publisher' : 'listener';
  ...
  const roomId = c.env.AUDIO_ROOM.idFromName(sessionId);
  return c.env.AUDIO_ROOM.get(roomId).fetch(c.req.raw);
});
